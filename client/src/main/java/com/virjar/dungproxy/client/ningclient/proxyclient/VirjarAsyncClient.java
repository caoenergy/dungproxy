...
public class VirjarAsyncClient {
	...
	<T> ListenableFuture<T> privatePost(final String url, Map<String, String> params, HttpOption option,
	...
	private static final Supplier<NioClientSocketChannelFactory> CHANNEL_FACTORY = Suppliers
			.memoize(new Supplier<NioClientSocketChannelFactory>() {
				...
				public NioClientSocketChannelFactory get() {
					return new NioClientSocketChannelFactory(ManagedExecutors.getExecutor(), ManagedExecutors
							.getExecutor(), Runtime.getRuntime().availableProcessors());
				}
			});
	...
	private final Supplier<AsyncHttpClient> lazyClient = Suppliers.memoize(new Supplier<AsyncHttpClient>() {
		...
		public AsyncHttpClient get() {
			builder.setExecutorService(ManagedExecutors.newFixedThreadPool(
					Runtime.getRuntime().availableProcessors() * 2, fact));
