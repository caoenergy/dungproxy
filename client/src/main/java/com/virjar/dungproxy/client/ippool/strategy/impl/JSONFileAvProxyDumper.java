...
public class JSONFileAvProxyDumper implements AvProxyDumper {
	...
	public Map<String, List<AvProxyVO>> unSerializeProxy() {
		...
		try {
			JSONObject jsonObject = JSONObject.parseObject(Files.toString(new File(trimFileName()),
					Charset.defaultCharset()));
			if (jsonObject == null) {
				log.warn("本地代理IP池序列化文件损坏");
				return ret;
			}

			for (Map.Entry<String, Object> entry : jsonObject.entrySet()) {
				ret.put(entry.getKey(), Lists.transform(
						JSONArray.class.cast(entry.getValue())
								.subList(0, JSONArray.class.cast(entry.getValue()).size()),
	...
	private String trimFileName() {
		try {
			...
			log.info("json序列化器,文件路径为:{}", dumpFileName);
		}
		return dumpFileName;
	}
