...
public class DefaultResourceFacade implements ResourceFacade {
	...
	public List<AvProxyVO> importProxy(String domain, String testUrl, Integer number) {
		...
		nameValuePairBuilder.addParam("checkUrl", testUrl).addParam("domain", domain)
				.addParam("num", String.valueOf(number));
		List<NameValuePair> valuePairList = nameValuePairBuilder.build();

		log.info("默认IP下载器,IP下载URL:{}", avUrl);
		...
		if (StringUtils.isBlank(response)) {
			log.error("can not get available ip resource from server: request body is {}",
					JSONObject.toJSONString(valuePairList));
			return Lists.newArrayList();
		}
		JSONObject jsonObject = JSONObject.parseObject(response);
		if (!jsonObject.getBoolean("status")) {
			log.error("can not get available ip resource from server:request  body is  last response is ",
	...
	public List<AvProxyVO> allAvailable() {
		log.info("默认IP下载器,IP下载URL:{}", allAvUrl);
		HttpClientContext httpClientContext = HttpClientContext.create();
		PoolUtil.disableDungProxy(httpClientContext);
		String response = HttpInvoker.get(allAvUrl, httpClientContext);
		if (StringUtils.isBlank(response)) {
			log.error("can not get available ip resource from server: ");
			return Lists.newArrayList();
		}
		JSONObject jsonObject = JSONObject.parseObject(response);
		if (!jsonObject.getBoolean("status")) {
			log.error("can not get available ip resource from server:request  body is  last response is {} ", response);
			return Lists.newArrayList();
		}
		return convert(jsonObject.getJSONArray("data"));
	}
